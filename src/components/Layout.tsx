...
const FALLBACK_AVATAR =
  "https://ui-avatars.com/api/?name=User&background=0B6B6E&color=fff";
  ...
  return (
    <div className="min-h-screen bg-brand-surface flex flex-col md:flex-row">
      {isInstallable && !dismissed && (
        <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-96 bg-slate-900 text-white rounded-xl shadow-lg p-4 flex items-center gap-3 z-50">
          ...
          <div className="flex-1">
            <p className="font-semibold text-sm">Install MeCure Academy</p>
      ...
      <main className="flex-1 md:ml-24 pb-20 md:pb-0">
        <header className="bg-white/80 backdrop-blur-md sticky top-0 z-20 px-6 py-4 border-b border-slate-100 flex justify-between items-center gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <img
              src="/img/mecure-industries-logo.png"
              alt="MeCure Excellence Academy"
              className="h-20 w-auto object-contain shrink-0 md:hidden"
            />
            <h1 className="md:text-xl text-lg font-bold text-slate-800 truncate">
              {activeItem.title}
            </h1>
          </div>
